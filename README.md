...
- workflows are directed acyclic graphs of `task`, `condition`, and bounded
  `loop` nodes;
- a workflow may declare named versions; every version is kept and executions
  are bound to the version that was current (or explicitly named) when they
  started, so an upgrade never changes a running execution;

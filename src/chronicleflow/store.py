...
# Columns added after the initial version, applied additively to databases
...
class Store:
        ...
        for row in rows:
            ...
            if changed:
                self.connection.execute(
                    ...
                    (json.dumps(document, ensure_ascii=False, separators=(",", ":"), sort_keys=True), row["rowid"]),
                )

    def _backfill_versions(self) -> None:
        """Give workflows created before versioning an unversioned revision row."""
        self.connection.execute(
    ...
    def _migrate_added_columns(self) -> None:
        """Add columns introduced after a table first existed to an existing file."""
        for table, (column, declaration) in ADDED_COLUMNS.items():
            columns = [
                info["name"]
                for info in self.connection.execute(f'PRAGMA table_info("{table}")')
            ]
